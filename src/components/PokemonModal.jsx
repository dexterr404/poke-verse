...
function PokemonModal({ pokemon, onClose, setPickedPokemon, handleTypeClick }) {
  const { user } = useAuth();
  const {addToPokemonCollection} = usePokemonContext();
  ...
  switch (pokemon.rarity) {
    case "mythical": return 100; // 1 in 100
    case "legendary": return 50; // 1 in 50
    case "rare": return 20;      // 1 in 20
    case "uncommon": return 10;  // 1 in 10
    default: return 5;           // Common: 1 in 5
  }
};

  const handleCatch = () => {
    if (!user) {
      alert("Please log in to catch Pokémon!");
      return;
    }
    else {
      setThrowCount((c) => c + 1);
      ...
      if (success) {
        addToPokemonCollection(pokemon);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs bg-opacity-50 flex items-center justify-center z-200">
          ...
          <div className="relative flex flex-col justify-center gap-2 items-center">
            ...
            {pokemon.types?.map((typeObj, index) => {
              ...
              return (
                ...
                >
                  <i className={`${iconClass} text-gray-800`}></i> <span className="capitalize">{type}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="pt-4">
            ...
            <div className="grid grid-cols-3 grid-rows-2 gap-3 mt-2 text-xs max-md:px-4 capitalize">
              <div className="flex flex-col">
                <div className="bg-gray-500 text-white px-2 pt-1 mb-1 rounded-t-[5px]">
                  Height
                </div>
                <div>
                  {pokemon.height}m
      ...
      <EvolutionChain stages={pokemon.evolutions} setPickedPokemon={setPickedPokemon}/>
